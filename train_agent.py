...
invalid_action_shape = (mapsize, envs.action_space.nvec[0:7].sum() + 1)
...
for update in range(starting_update, num_updates + 1):
    ...
    for step in range(0, args.num_steps):
        ...
        debug('valid_actions: ', valid_actions, valid_actions.shape)
        valid_actions_counts = invalid_action_masks[step][:, :, 0].sum(1).long().cpu().numpy()
        debug('valid_actions_counts: ', valid_actions_counts, valid_actions_counts.shape)
        #java_valid_actions = []
        #valid_action_idx = 0
        ...
        with torch.no_grad():
            try:
                ...
                raw_obs, rs, ds, infos = envs.step(flatten_action)
                next_obs, next_entity_mask, next_entity_count, next_unit_position, next_unit_mask, next_enemy_unit_mask, next_neutral_unit_mask = \
